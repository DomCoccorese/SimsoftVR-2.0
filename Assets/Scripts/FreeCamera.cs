...
public class FreeCamera : MonoBehaviour
{
    ...
    private bool AnchoredMovementEnabled = false;
    private bool AnchoredRotationEnabled = false;
    ...
    private void LateUpdate()
    {
        ...
        if (AnchoredMovementEnabled && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject())
        {
            move += Vector3.up * Mouse.current.delta.y.ReadValue() * -moveSpeed;
            move += Vector3.right * Mouse.current.delta.x.ReadValue() * -moveSpeed;
        }

        //Rotate the camera when anchored
        if (AnchoredRotationEnabled && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject())
        {
            transform.RotateAround(transform.position, transform.right, Mouse.current.delta.y.ReadValue() * -rotationSpeed);
            transform.RotateAround(transform.position, Vector3.up, Mouse.current.delta.x.ReadValue() * rotationSpeed);
        }
    ...
    {
        transform.Translate(Vector3.forward * context.ReadValue<float>() * zoomSpeed);
    }

    public void EnableAnchoredMove(InputAction.CallbackContext context)
    {
        AnchoredMovementEnabled = context.ReadValue<float>() > 0;
    }

    public void EnableAnchoredRotate(InputAction.CallbackContext context)
    {
        AnchoredRotationEnabled = context.ReadValue<float>() > 0;
    }
}